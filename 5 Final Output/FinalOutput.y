...
assignment : vari ASSIGN expression {check(temp);setValue(temp,$<t.v.ival>3);addQuadruple(temp,"","",pop());pop();} 
	;
...
writing : WRITE CBO towrite CBC
	| WRITE CBO DOUBLEQ toprint DOUBLEQ CBC {printf("\n");}
	;

towrite : towrite COMMA ID {
		check($<t.varname>3);
		int out=getValue($<t.varname>3);
		printf("%d\n",out);}

	| ID {
		check($<t.varname>1);
		int out=getValue($<t.varname>1);
		printf("%d\n",out);
    }

toprint : INWORDS {printf("%s",$<t.varname>1);}
	| toprint INWORDS {printf("%s",$<t.varname>2);}
    ;

simple_if : IF cond THEN blocks
	;

if_then_else : IF cond THEN blocks elsecond 
	;

elsecond: ELSE blocks 
	;

while_do : WHILE cond DO blocks 
	;

for_do : FOR assignment to expression DO blocks
    | FOR assignment downto expression DO blocks 
	;
...
int main(int argc, char *argv[]){
	...
	int i=0;
	for(i=0; i<count; i++) {
		printf("%-*s %-*s %-*s\n", max_length,symbol_table[i].id_name, max_length, symbol_table[i].data_type, max_length, symbol_table[i].value);
	}
	for(i=0;i<count;i++) {
